...
def get_connections(status_filter):
    ...
    for conn in psutil.net_connections(kind='inet'):
        if conn.status == status_filter:
            ...
            pid = conn.pid if conn.pid else "root only"
            program = get_process_name(conn.pid).ljust(20) if conn.pid else "root only".ljust(20)
            user = get_process_user(conn.pid).ljust(15) if conn.pid else "root only".ljust(15)
            connections.append([laddr, raddr, status, str(pid).ljust(8), program, user])
    return connections
...
def main(stdscr):
    ...
    stdscr.timeout(500)  # Refresh every 500 ms for smoother updates
    ...
    def update_display():
        ...
        for conn in established_connections + listening_connections:
            pid = conn[3].strip()
            if pid != "root only" and pid.isdigit():
                pid = int(pid)
                try:
        ...
        draw_table(buffer, "Established Connections", [
            conn + [format_size(io_data.get(int(conn[3].strip()), {}).get('sent', 0)) if conn[3].strip().isdigit() else "root only", format_size(io_data.get(int(conn[3].strip()), {}).get('recv', 0)) if conn[3].strip().isdigit() else "root only"]
